...
execute as @e[tag=Henri,limit=1,sort=random] at @a[tag=HenriTravellocation,limit=1,sort=random,distance=30..] unless data storage minecraft:henridatapack {autospawn:0} run tp @s ~ ~ ~
tag @a[tag=HenriTravellocation] remove HenriTravellocation
execute unless data storage minecraft:henridatapack {autospawn:0} run schedule function function henris_potions:henri/travel 2d
